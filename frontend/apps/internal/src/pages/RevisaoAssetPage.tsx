      ...
      <Card>
        <CardContent className="p-3">
          ...
          {url && asset.media_type === "video" && (
            <video ref={mediaRef} src={url} controls className="w-full rounded-md" />
          )}
      ...
      {!isReadOnly && (
        <Card>
          ...
          <CardContent className="space-y-2">
            ...
            />
            <Button size="sm" loading={addComment.isPending} onClick={submitComment}>
              Comentar
            </Button>
          </CardContent>
        </Card>
            ...
            <ul className="divide-y divide-line">
              {commentsQuery.data.map((c) => (
                <li key={c.id} className="space-y-1 py-2">
                  ...
                  <p className={`text-sm ${c.resolved ? "text-muted line-through" : "text-ink"}`}>
                    {c.body}
                  </p>
        ...
        <Card>
          ...
          <CardContent>
            <ul className="divide-y divide-line text-sm">
              {history.map((v) => (
                <li key={v.version_number} className="flex items-center justify-between py-1.5">
                  <span className="text-ink">Versão {v.version_number}</span>
                  <Link
                    to={`/revisao/${spaceId}/asset/${assetId}?v=${v.version_number}`}
