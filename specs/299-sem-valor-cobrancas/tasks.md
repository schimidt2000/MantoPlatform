...
- [X] T064 No outro evento de um grupo cujo principal está sem valor, mostrar "Recebido no grupo R$ X
...
- [X] T065 No principal sem valor, mostrar também "Inclui R$ X em comprovantes de outros eventos do
...
- [X] T066 Em `update_event_comercial` (`app/calendar/event_ops.py:1084-1086`), manter a data da venda
  já gravada quando o valor fica vazio (passar `a_definir` ao `resolver_data_da_venda`, fora a
  cortesia), para nenhuma edição da aba Comercial apagar a data per FR-017 (partial) *(Cenário 5,
  (d').)*
- [X] T067 Trocar a adivinhação de `_data_herdada_da_educamanto` (`app/financeiro/comissoes_ops.py`,
  ...
  (contradicts)
- [X] T068 Levar para o FR-031 a regra "cortesia nunca comissiona, mesmo a antiga com valor gravado"
...
- [X] T069 Corrigir no `docs/02_MAPA_DE_PAGINAS_E_UX.md` a frase `:464` ("Só quem pode gerir a venda
...
- [X] T071 Tirar a marca `NOTA_CICLO_VALOR_TARDIO` das notas quando o ramo EducaManto de
...
- [X] T072 No chip "Recebimento" do Resumo
...
- [X] T073 Acrescentar ao `docs/02_MAPA_DE_PAGINAS_E_UX.md` (bloco "Feature 299 na aba Comercial",
...
- [X] T074 Corrigir no `specs/299-sem-valor-cobrancas/spec.md` o FR-031 (`:526-527`: a comissão de
...
- [X] T075 Corrigir nos artefatos a exceção do valor mantido, que vale só para o mesmo valor
