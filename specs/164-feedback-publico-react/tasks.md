...
- [X] T001 [P] Adicionar rota `/avaliar/:token` (placeholder) em
...
- [X] T002 [US1] Criar `app/api/feedback_write.py` (NOVO): importa `POSITIVE_TAGS`,
...
- [X] T003 [US1] Importar `feedback_write` em `app/api/__init__.py` (mesmo padrão dos demais
      módulos `_write`).
- [X] T004 [P] [US1] Criar `frontend/apps/public/src/lib/feedback.ts`: tipos (`FeedbackEvent`) +
      hooks `useFeedbackEvent(token)` (`useQuery`) e `useSubmitFeedback(token)` (`useMutation`,
      JSON body).
- [X] T005 [P] [US1] Criar `frontend/apps/public/src/components/feedback/StarRating.tsx`: 5
      botões preenchendo até o valor sob hover ou até `score` (`research.md` §3), `onChange
      (score: number)`, alvo de toque ≥44px.
- [X] T006 [P] [US1] Criar `frontend/apps/public/src/components/feedback/TagChips.tsx`: chips de
...
- [X] T007 [US1] Criar `frontend/apps/public/src/pages/AvaliarPage.tsx`: busca o evento pelo
...
- [X] T008 [US1] Em `App.tsx` (T001), substituir o placeholder de `/avaliar/:token` por
      `AvaliarPage` (T007).
...
- [X] T009 Criar `scripts/db/verify_164_feedback_publico_react.py` (gitignored): test client
...
- [X] T010 Rodar `ruff check app/api/feedback_write.py`.
- [X] T011 Rodar `npm run typecheck:public` e `npm run build:public`.
- [X] T012 Conferência mobile (320–430px) dos 3 estados da tela — Princípio VIII. **Não
      verificado visualmente nesta sessão**: sem Playwright/chromium-cli disponível no ambiente
      (mesma limitação recorrente das fatias 156-163). Layout mobile-first (`Card` em coluna
      única, botões de estrela `h-11 w-11` ≥44px, chips com `flex-wrap`); recomenda-se
      conferência visual manual em viewport real.
- [X] T013 Atualizar `docs/changelog.html` com entrada em linguagem simples (entrada 164,
