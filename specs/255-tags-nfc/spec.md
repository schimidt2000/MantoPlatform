...
5. **Associação ao evento OU à cliente direta**: no fluxo principal a tag aponta para o evento do show e a cliente vem de carona pelo evento. Mas a luminária também vai para **clientes em potencial** (campanhas de marketing, brindes) — sem show nenhum: nesse caso a equipe cria/acha a cliente no módulo Clientes e vincula a tag **direto à cliente**. Cliente direta tem precedência sobre a do evento na exibição. Ambas as associações podem ser feitas/trocadas depois.
...
- **FR-004b**: A equipe MUST poder vincular uma tag **diretamente a uma cliente** (sem evento) — o caso da campanha de marketing/brinde para cliente em potencial. A cliente exibida na lista é a direta quando houver; senão, a contratante do evento. Ambos os vínculos são alteráveis e independentes.
...
- **FR-006**: A página pública MUST apresentar identidade Manto com uma animação de entrada impactante **temática da luminária real** (a peça é uma estrela "Magia de Sonhar" com o Mantinho sentado em cima e uma nuvem "Manto Produções" na base): céu noturno com nuvens, a estrela **acendendo** como a luminária física acende — revelando o "Magia de Sonhar" —, respeitando preferência de movimento reduzido (tudo visível e aceso, sem coreografia). Texto de boas-vindas e link para o Instagram da Manto. A arte da estrela substitui a foto do produto do acervo na v1.
