...
- [X] T001 [P] Adicionar rotas `/f/pre-contrato`, `/f/corporativo` e `/f/:formType/enviado`
      (placeholders) em `frontend/apps/public/src/App.tsx`, substituídos nas fases seguintes.

## Phase 2: Foundational

- [X] T002 Criar `app/api/formularios_write.py` (NOVO): importa (não copia) `FORM_META`,
...
- [X] T003 Importar `formularios_write` em `app/api/__init__.py` (mesmo padrão dos demais
      módulos `_write`).
- [X] T004 [P] Criar `frontend/apps/public/src/lib/formularios.ts`: tipos
...
- [X] T005 [US1] Criar `app/api/formularios_write.py` — `GET /api/formularios/<form_type>/schema`
...
- [X] T006 [US1] Criar `frontend/apps/public/src/components/formularios/DynamicField.tsx`:
...
- [X] T007 [US1] Criar `frontend/apps/public/src/components/formularios/DynamicForm.tsx`: busca
...
- [X] T008 [P] [US1] Criar `frontend/apps/public/src/pages/FormularioPage.tsx`: lê `formType`
      da rota (`comum` para `/f/pre-contrato`, `corporativo` para `/f/corporativo`), monta
      `DynamicForm` (T007) num layout mobile-first.
- [X] T009 [P] [US1] Criar `frontend/apps/public/src/pages/FormularioEnviadoPage.tsx`: paridade
...
- [X] T010 [US1] Em `App.tsx` (T001), substituir os placeholders de `/f/pre-contrato`,
...
- [X] T011 [US2] Em `DynamicForm.tsx` (T007), no `onBlur` de qualquer campo `type === "cep"` com
...
- [X] T012 Criar `scripts/db/verify_163_formularios_dinamicos_react.py` (gitignored): test client
...
- [X] T013 Rodar `ruff check app/api/formularios_write.py`.
- [X] T014 Rodar `npm run typecheck:public` e `npm run build:public`.
- [X] T015 Conferência mobile (320–430px) das 2 telas de formulário e da tela de confirmação —
      Princípio VIII. **Não verificado visualmente nesta sessão**: sem Playwright/chromium-cli
      disponível no ambiente (mesma limitação recorrente das fatias 156-162). Layout construído
      mobile-first (`Card` em coluna única, mesmos componentes/breakpoints do `CadastroForm` da
      162); recomenda-se conferência visual manual em viewport real antes do próximo passo.
- [X] T016 Atualizar `docs/changelog.html` com entrada em linguagem simples (entrada 163) e
