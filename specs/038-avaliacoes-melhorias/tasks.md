...
- [ ] T001 `app/talents/routes.py`: helper `_parse_period()` + leitura/validação de `cat`, `period`,
      `from`, `to` (inválidos → fallback); base de ratings com join em CalendarEvent por período.
- [ ] T002 Recorte por categoria: scores/dist/comentários de `EventSubRating` quando `cat` ativo;
      manter visão geral atual quando não.
- [ ] T003 Seletor agrupado por mês (`event_groups`); comentários unificados (geral + categoria, com
      `cat_label` e `subject_name`); ranking best/worst; pontos de atenção (≤2, limite 10); tendência
      mensal (média + contagem).

## Phase 2: Template
- [ ] T004 `avaliacoes.html`: barra de filtros (período com atalhos + personalizado, categoria,
      evento com optgroup por mês, limpar filtros), auto-submit, filtros preservados na URL.
- [ ] T005 Painéis novos: tendência (barras CSS), ranking melhores/piores (links preservando `cat`),
...
## Phase 3: Verificação
- [ ] T006 `ruff check` + boot + test client: sem filtros; `cat=figurino`; `period=30d`; custom
