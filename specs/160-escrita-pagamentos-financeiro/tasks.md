...
- [X] T002 Criar `app/api/financeiro_write.py` (NOVO): imports/estrutura base (`api_bp`,
...
- [X] T003 [P] Adicionar `apiFetchBlob(path, options)` em
...
- [X] T004 [US1] Implementar `POST /api/financeiro/pagamentos/set-status` em
...
- [X] T005 [P] [US1] Criar `useSetPaymentStatus()` em
...
- [X] T006 [US1] Em `frontend/apps/internal/src/pages/PagamentosPage.tsx`, trocar o badge de status
...
- [X] T007 [US2] Implementar `POST /api/financeiro/pagamentos/bulk-action` em
      ...
      ...]}`.
- [X] T008 [P] [US2] Criar `useBulkPaymentAction()` em
...
- [X] T009 [US2] Em `PagamentosPage.tsx`: checkbox por linha + estado de seleção (por
...
- [X] T010 [US3] Implementar `POST /api/financeiro/pagamentos/salary/<int:sp_id>/advance` em
      ...
      `sp_id` não existe.
- [X] T011 [US3] Implementar `POST /api/financeiro/pagamentos/salary/advance/<int:adv_id>/delete`
...
- [X] T012 [P] [US3] Criar `useAddSalaryAdvance()`/`useDeleteSalaryAdvance()` em
...
- [X] T013 [US3] Em `PagamentosPage.tsx`, dentro do `<details>` de adiantamentos de um item
...
- [X] T014 [US4] Implementar `GET /api/financeiro/pagamentos/export` em
...
- [X] T015 [US4] Em `PagamentosPage.tsx`, botão "Exportar CSV" no cabeçalho (ao lado do seletor de
...
- [X] T016 Criar `scripts/db/verify_160_escrita_pagamentos_financeiro.py` (gitignored): test
...
- [X] T017 Rodar `ruff check app/` nos arquivos tocados.
- [X] T018 Rodar `npx tsc --noEmit` e `npm run build` em `frontend/apps/internal`.
- [ ] T019 Conferência mobile (320–430px) das novas ações (checkbox + barra de ação em massa,
      formulário de adiantamento, botão de exportar) — Princípio VIII. **Não verificado nesta
      sessão**: sem Playwright/chromium-cli disponível no ambiente (mesma limitação da 156-159).
      A tabela já usa `overflow-x-auto` (padrão herdado), e os controles novos (checkbox, select,
      barra de ação, formulário) usam classes flex-wrap/tamanho relativo consistentes com o resto
      da tela; recomenda-se conferência visual manual antes do merge, se possível.
- [X] T020 Atualizar `docs/changelog.html` com entrada em linguagem simples (entrada 160) e
