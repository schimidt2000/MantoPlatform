...
- [ ] T001 `app/talent_portal/routes.py`: helpers `_now_sp()` e `_event_ended(event)`.
- [ ] T002 Trocar `utcnow()` por `_now_sp()` nas comparações com horários de evento:
...
- [ ] T003 `rate_event` (GET), `submit_rating` (POST), `rate_event_detail` (GET/POST): recusar
      evento não terminado com flash + redirect ao home do portal.

## Phase 3: Verificação
- [ ] T004 ruff + boot + test client: evento em +2h não listado/recusado (GET redirect, POST não
