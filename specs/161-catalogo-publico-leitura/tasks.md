...
- [X] T001 Adicionar ao `frontend/apps/public/package.json` as dependências já usadas em
...
- [X] T002 [P] Criar `frontend/apps/public/postcss.config.js` (idêntico ao de `apps/internal`).
- [X] T003 [P] Criar `frontend/apps/public/tailwind.config.ts`: tokens portados de
...
- [X] T004 [P] Criar `frontend/apps/public/src/index.css`: diretivas `@tailwind`
...
- [X] T005 Reescrever `frontend/apps/public/src/main.tsx`: remover o placeholder "Em
...
- [X] T006 Criar `frontend/apps/public/src/App.tsx` com `BrowserRouter`/`Routes` — rotas `/`,
...
- [X] T007 Adicionar ao `frontend/package.json` raiz os scripts `dev:public`, `build:public`,
...
- [X] T008 Criar `app/api/catalogo_read.py` (NOVO): 4 endpoints públicos (sem
      `@login_required`) —
...
- [X] T009 Importar `catalogo_read` em `app/api/__init__.py` (mesmo padrão dos demais módulos
      `_read`).
- [X] T010 [P] Criar `frontend/apps/public/src/lib/catalogo.ts`: tipos TS (`CatalogItemSummary`,
...
- [X] T011 [P] Criar `frontend/apps/public/src/components/ProductCard.tsx`: card reutilizável
...
- [X] T012 [US1] Criar `frontend/apps/public/src/pages/CatalogGridPage.tsx`: usa
...
- [X] T013 [P] [US1] Criar `frontend/apps/public/src/pages/CategoriesPage.tsx`: usa
      `useCategories()` (T010); grid de cards de categoria (foto de capa + overlay com nome e
      contagem); estado vazio amigável.
- [X] T014 [P] [US1] Criar `frontend/apps/public/src/pages/CategoryDetailPage.tsx`: usa
...
- [X] T015 [US1] Criar `frontend/apps/public/src/components/ProductGallery.tsx`: galeria
...
- [X] T016 [US1] Criar `frontend/apps/public/src/pages/ProductDetailPage.tsx`: usa
...
- [X] T017 [US1] Em `App.tsx` (T006), substituir os placeholders de `/`, `/categorias`,
...
- [X] T018 [US2] Criar `frontend/apps/public/src/lib/wishlist.ts`: porta
...
- [X] T019 [P] [US2] Criar `frontend/apps/public/src/components/WishlistButton.tsx`: usa
...
- [X] T020 [P] [US2] Criar `frontend/apps/public/src/components/WishlistFloat.tsx`: botão
...
- [X] T021 [US2] Criar `frontend/apps/public/src/pages/WishlistPage.tsx`: lista os itens de
...
- [X] T022 [US2] Em `App.tsx`, substituir o placeholder de `/lista-desejos` por `WishlistPage`
...
- [X] T023 Criar `scripts/db/verify_161_catalogo_publico_leitura.py` (gitignored): test client
...
- [X] T024 Rodar `ruff check app/api/catalogo_read.py`.
- [X] T025 Rodar `npm run typecheck:public` e `npm run build:public`.
- [ ] T026 Conferência mobile (320–430px) das 5 telas — grade, categorias, categoria, detalhe
      ...
      ficar atrás do teclado virtual) — Princípio VIII. **Não verificado visualmente nesta
      sessão**: sem Playwright/chromium-cli disponível no ambiente (mesma limitação recorrente
      das fatias 156-160). O layout foi construído mobile-first desde o início (grid 2 colunas
      em telas pequenas, tipografia `clamp()`, botão flutuante `fixed` testado só por leitura de
      CSS) e o proxy Vite→Flask foi validado ponta a ponta com dados reais de `manto_local`
      (smoke test manual desta sessão); recomenda-se conferência visual manual em viewport
      real antes do próximo passo da US5.
- [X] T027 Atualizar `docs/changelog.html` com entrada em linguagem simples (entrada 161) e
