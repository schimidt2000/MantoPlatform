...
> ganha **deep link** (`?destino=`) para o botão do e-mail de reenvio cair na tela certa.
> Antes: **272-notificacoes-internas** (sino de notificações no shell + `/notificacoes`). Antes: **270-miniaturas-catalogo** (variantes de miniatura na vitrine e no
...
- **Layout (feature 215 — substitui o mural de duas colunas da 190)**: **quatro abas**
  ...
  - **Aba Comercial**: *Clientes* (**editável inline**) · *Pré-contrato* (**editável inline**) ·
    *Comercial — dados da venda* (**editável inline**: bruto/desconto/final, transporte, comissão,
    ...
    marcação de BV seguem em leitura) · painel **"Orçamento"** (feature 273, sucessor da linha
    "Orçamento de origem" da 239): com orçamento vinculado, chips do que foi vendido ("Fora de SP ·
    110 km", "2 coordenadores", "Maquiagem em 2", "Show · técnico de som"), "Abrir orçamento",
    **"Aplicar ao evento"** (equipe + fora de SP; nunca apaga nem rebaixa), "Trocar" e
    "Desvincular", e a frase do que a aplicação fez; sem orçamento, `OrcamentoPicker` (busca em
    `/api/orcamento/historico?q=`, que desde a feature 301 devolve os orçamentos de todo o comercial —
    cada linha do resultado mostra de quem é) e, em evento **sem venda**, a
    ...
    FINANCEIRO vê o painel só para leitura. **Feature 301**: o resumo do orçamento passa a vir para
    todo o comercial, mesmo quando o orçamento é de outra pessoa — o painel mostra os chips, **de
    quem é** e "Abrir orçamento". Quem pode *mexer* no vínculo é dito pelo servidor em
    `venda.orcamento.pode_gerir` (autor do orçamento vinculado, ou superadmin): sem essa flag,
    "Aplicar ao evento", "Trocar" e "Desvincular" não são renderizados, e no lugar deles aparece a
    frase que nomeia o autor e a saída. A tela **não deduz** mais permissão da ausência do
    orçamento no payload — com a visibilidade restaurada essa dedução seria sempre falsa e os três
    botões apareceriam para todo mundo, para o servidor recusar depois do clique. `tem_orcamento`
    continua para quem não tem o módulo (FINANCEIRO): "vinculado a um orçamento que o seu perfil
    não abre". O 409 "já vinculado a outro evento" vem com link para esse evento ·
...
| `/orcamento/historico` | Orçamentos | `COMERCIAL`, `SUPERADMIN` | tabela densa com filtros avançados (data, valor, vendedor, tipo), PDF, envio por e-mail, exclusão; **Abrir orçamento** (`/orcamento/:id`), **Criar evento** (`/events/new?orcamento_id=`) e **Recalcular** (`/orcamento?recalcular_id=`), feature 190. **Feature 273**: linha vinculada a um evento vivo mostra **Ver evento** (`/events/:id`); excluir orçamento vinculado é recusado com 409 e a mensagem, mostrada na página com link "Ver evento", aponta o evento a desvincular. **Feature 301**: a tabela passou a ser **do time inteiro** — a coluna **Vendedor** e o filtro por vendedor, que só o superadmin via, agora aparecem para todo o comercial, e a lista **abre sem filtro** (não começa em "meus orçamentos"). O botão **Excluir** só é renderizado na linha de quem pode excluir (autor ou superadmin), por `pode_excluir` do payload — antes aparecia em toda linha e só falhava depois do `confirm()`. Com 1.809 orçamentos concentrados (90% de um vendedor) e teto de 300 linhas, os filtros passam a ser o caminho normal |
